...
public class ExerciseTypeService : IExerciseTypeService
{
    ...
    public async Task<DomainResponse<QueryResponse<ExerciseType>>> GetAsync(QueryRequest query)
    {
        ...
        return DomainResponse.Success(
            new QueryResponse<ExerciseType>
            {
                Data = result,
    ...
    /// <returns>A task representing the asynchronous operation. The task result contains a DomainResponse object containing the retrieved ExerciseType entity.</returns>
    public async Task<DomainResponse<ExerciseType>> GetByIdAsync(int id)
    {
        if (id <= 0)
        {
            _logger.LogWarning(ServiceMessages.EntityIdInvalid<ExerciseType>());
            return DomainResponse.Failure<ExerciseType>(EntityErrorMessage<ExerciseType>.BadRequest());
        }
        ...
        if (exerciseType == null)
        {
            ...
            return DomainResponse.Failure<ExerciseType>(EntityErrorMessage<ExerciseType>.NotFound(id));
        }
           
        _logger.LogInformation(ServiceMessages.EntityRetrieved<ExerciseType>(exerciseType.Id));
        return DomainResponse.Success(exerciseType);
    ...
    /// <returns>A Task representing the asynchronous operation. The Task's result contains a DomainResponse<int> object that represents the result of the creation operation.</returns>
    public async Task<DomainResponse<int>> CreateAsync(ExerciseType entity)
    {
        ...
        if (!validationResult.IsValid)
        {
            ...
            return DomainResponse.Failure<int>(EntityErrorMessage<ExerciseType>.BadRequest(validationResult.ToDictionary()));
        }
    ...
    /// <returns>A Task that represents the asynchronous operation. The task result contains a DomainResponse<bool> indicating the success or failure of the update operation.</returns>
    public async Task<DomainResponse<bool>> UpdateAsync(ExerciseType entity)
    {
    ...
    /// <returns>A task representing the asynchronous operation. The task result contains a DomainResponse with a boolean value indicating the success of the delete operation.</returns>
    public async Task<DomainResponse<bool>> DeleteAsync(int id)
    {
        if (id <= 0)
        {
            _logger.LogWarning(ServiceMessages.EntityIdInvalid<ExerciseType>());
            return DomainResponse.Failure<bool>(EntityErrorMessage<ExerciseType>.BadRequest());
        }
