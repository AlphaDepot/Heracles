...
function DragHandler(active, axis, rootNode, target, dragTracker, location, animation, scrollTo2, scrollBody, scrollTarget, indexCurrent, eventHandler, percentOfView, dragFree, dragThreshold, skipSnaps, baseFriction) {
  const {
    cross: crossAxis,
    direction
  } = axis;
  ...
  function up(evt) {
    ...
    scrollTo2.distance(force, !dragFree);
    event.emit();
  }
  function click(evt) {
    if (preventClick) {
...
function SlideFocus(axis, active, root, slides, scrollSnapList, scrollTo2, scrollBody, eventStore, eventHandler) {
...
function Engine(root, container, slides, options, nodeHandler, eventHandler, rects, isSsr) {
  ...
  const scrollTo2 = ScrollTo(animation, indexCurrent, indexPrevious, scrollBody, scrollTarget, target, eventHandler);
  const eventStore = EventStore();
  const slidesInView = SlidesInView(container, slides, eventHandler, inViewThreshold, inViewMargin);
  const slideFocus = SlideFocus(axis, focus, root, slides, scrollSnapList, scrollTo2, scrollBody, eventStore, eventHandler);
  const engine = {
    eventHandler,
    containerRect,
    contentSize,
    ...
    isSsr,
    axis,
    dragHandler: DragHandler(draggable, axis, root, target, DragTracker(axis), location, animation, scrollTo2, scrollBody, scrollTarget, indexCurrent, eventHandler, percentOfView, dragFree, dragThreshold, skipSnaps, friction),
    eventStore,
    percentOfView,
    indexCurrent,
    indexPrevious,
...
function NodeHandler(root) {
  ...
  function getSsrNodes(options) {
    const rootSize = 100;
    const root2 = createSsrNode(0, 0, rootSize, rootSize);
    const container = root2;
    let startOffset = 0;
...
function SsrHandler(container, axis, nodeHandler, options, mergeOptions, createEngine) {
  const translate = Translate(axis, container, "%");
  function createStyles(options2, containerSelector, slidesSelector) {
    const {
      direction
    } = Axis(options2.axis, options2.direction);
    const {
      slides,
      container: container2
    } = nodeHandler.getNodes(options2);
    const {
      location,
      slideLooper,
      contentSize
    } = createEngine(options2, container2, slides);
    const loopPoints = options2.loop ? slideLooper.loopPoints : [];
...
function EmblaCarousel(userRoot, userOptions, userPlugins) {
  ...
  function createEngine(options2, container2, slides2, useCachedRects) {
    ...
    const engineOptions = mergeOptions(options2, ssrOptions);
    const rects = nodeHandler.getRects(container2, slides2, useCachedRects);
    const engine2 = Engine(root, container2, slides2, engineOptions, nodeHandler, eventHandler, rects, isSsr);
    if (options2.loop && !engine2.slideLooper.canLoop()) {
      const optionsWithNoLoop = mergeOptions(options2, {
        loop: false
      });
      return createEngine(optionsWithNoLoop, container2, slides2, true);
    }
    return engine2;
  }
  ...
  function ssrStyles(container2, slides2) {
    return isSsr ? ssrHandler.getStyles(container2, slides2) : "";
  }
...
function Autoplay(userOptions = {}) {
  ...
  function clearTimer() {
    ...
    if (!ownerWindow) return;
    const stopTime = (/* @__PURE__ */ new Date()).getTime();
...
// js/carousel.ts
var instances = {};
function notifyDotNet(id) {
  void DotNet.invokeMethodAsync("Heracles.Blazor", "EmblaStateChanged", id);
}
var pluginRegistry = {
...
function initCarousel(id, viewport, options, plugins) {
  if (!viewport) return;
  const resolvedPlugins = (plugins || []).map(resolvePlugin).filter(Boolean);
  const api = EmblaCarousel(viewport, options, resolvedPlugins);
  instances[id] = api;
  const autoplay = resolvedPlugins.find(
    (p) => p && typeof p.play === "function"
  );
  if (autoplay) {
    autoplay.play();
  }
...
function canGoPrev(id) {
  const api = instances[id];
  return api ? api.canGoToPrev() : false;
}
function canGoNext(id) {
  const api = instances[id];
  return api ? api.canGoToNext() : false;
}
function getSelectedSnap(id) {
  const api = instances[id];
