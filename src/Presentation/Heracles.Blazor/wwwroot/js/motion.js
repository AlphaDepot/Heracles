...
var mask = {
  ...
  getAnimatableNone: (v) => {
    ...
    return transformer(parsed.map((v2) => typeof v2 === "number" ? 0 : typeof v2 === "object" ? {
      ...v2,
      alpha: 1
    } : v2));
  }
...
var motionInterop = {
  ...
  animate(element, keyframes2, options) {
    animate(element, keyframes2, options || {});
  },
  playExit(element) {
    return new Promise((resolve) => {
      const opts = element.__motion;
      if (!opts?.exit) {
        resolve();
        return;
      }
      animate(element, opts.exit, opts.transition || {}).finished.then(() => resolve());
    });
  },
  /** Presence: animate out old element, animate in new one */
  async swap(oldEl, newEl) {
    const oldOpts = oldEl.__motion;
    const newOpts = newEl.__motion;
    if (oldOpts?.exit) {
