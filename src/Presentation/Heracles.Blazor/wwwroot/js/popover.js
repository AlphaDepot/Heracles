...
async function initialize(id, options) {
  try {
    ...
    const arrowElement = popover2.querySelector('[data-slot="arrow"]');
    const ref = target && target.children.length === 1 ? target.firstElementChild : target;
    if (!ref) return;
    portal(popover2);
    ...
    const update = async () => {
      const data = await computePosition2(
        ref,
        popover2,
        {
          placement,
    ...
    await update();
    cleanupAutoUpdate = autoUpdate(ref, popover2, update);
  } catch (err) {
