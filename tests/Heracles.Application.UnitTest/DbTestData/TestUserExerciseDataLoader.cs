...
public abstract class TestUserExerciseDataLoader
{
	...
	public static List<UserExercise> UserExercises(AppDbContext context)
	{
		...
		[
			new UserExercise
			{
				UserId = users[0].UserId, ExerciseTypeId = 1, ExerciseType = exerciseTypes[0], Version = 1,
				CreatedAt = date, UpdatedAt = date
			},
			new UserExercise
			{
				UserId = users[0].UserId, ExerciseTypeId = 2, ExerciseType = exerciseTypes[1], Version = 1,
				CreatedAt = date, UpdatedAt = date
			},
			new UserExercise
			{
				UserId = users[0].UserId, ExerciseTypeId = 3, ExerciseType = exerciseTypes[2], Version = 1,
				CreatedAt = date, UpdatedAt = date
			},
			new UserExercise
			{
				UserId = users[1].UserId, ExerciseTypeId = 1, ExerciseType = exerciseTypes[0], Version = 2,
				CreatedAt = date, UpdatedAt = date
			}
		];
	}
