...
public class UpdateExerciseTypeCommandHandlerTest : HandlerBaseUnitTest
{
	...
	public void SetUp()
	{
		...
		_handler = new UpdateExerciseTypeCommandHandler(ExerciseTypesRepository);
	}
	...
	public async Task UpdateExerciseTypeCommandHandler_ShouldReturnSuccessResult()
	{
		...
		var storedExerciseType = await ExerciseTypesRepository.GetByIdAsync(exerciseType.Id);
		...
		var updatedExerciseType = await ExerciseTypesRepository.GetByIdAsync(exerciseType.Id);
	...
	{
		// Arrange
		var storedExerciseType = await ExerciseTypesRepository.GetByIdAsync(_exerciseTypes[2].Id);
